export default class BurgerMenu {
	...
	hideBurgerMenu(keepSwipePosition = false) {
		...
		if (!keepSwipePosition) {
			this.burgerMenu.removeEventListener('transitionend', this.onMenuTransitionEnd);
	...
	handleTouchMove(event) {
		if (!this.isSwiping || !this.isBurgerMenuOpen()) return;
	...
	handleTouchEnd(event) {
		...
		if (swipeDistance < -70) {
			this.burgerMenu.style.transition =
				'left var(--transition-time), opacity var(--transition-time), visibility var(--transition-time)';
			this.burgerMenu.addEventListener('transitionend', this.onMenuTransitionEnd);
